...
values <- list(mu = log(3), theta = 1/9)

lim <- prob.asym(n = 1000, values = values, v=0.4)[2]
lim <- ifelse(lim < 200,200,lim)
lim <- seq(0,lim,length=1000)


mat.pr <- sapply(c(0.1,0.2,0.3,0.4),function(i){
          sapply(lim,prob.asym,values = values,v=i)[1,]
        })
...
values <- list(mu = log(3), theta = 1)

lim <- unlist(prob.asym(n = 1000, values = values, v=0.4)[2])
lim <- seq(0,lim,length=1000)
